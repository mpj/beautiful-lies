...
// license: MIT
(function() {
  var args_as_array, arrays_equal, createAndInjectHandlers, create_liar, filter_on_args, filter_on_function, find_function, generateHandler, injectPlugins, is_function, lie, preprocessExpectation, remove_functions, run_delayed,
    __slice = [].slice;

  create_liar = function(expectations) {
    return createAndInjectHandlers({}, expectations);
  };

  create_liar.plugins = require('./plugins');

  lie = function(expectations) {
    var temp_obj;
    if (!Array.isArray(expectations)) {
      expectations = [expectations];
    }
    expectations.forEach(function(exp) {
      return exp.function_name = 'untitled_function';
    });
    temp_obj = create_liar(expectations);
    return temp_obj['untitled_function'];
  };

  createAndInjectHandlers = function(host, expectations) {
    ...
    for (_i = 0, _len = expectations.length; _i < _len; _i++) {
      ...
      host[expectation.function_name] = generateHandler(expectation.function_name, expectations);
  ...
  preprocessExpectation = function(expectation) {
    ...
    return injectPlugins(expectation);
  };

  injectPlugins = function(expectation) {
  ...
  generateHandler = function(function_name, all_expectations) {
    var handler;
    handler = function() {
      var expectation, expectations_matching, expectations_matching_args, handler_callback, m, match, message, process_result_spec, process_single_callback_spec, s, _base, _i, _j, _len, _len1, _ref, _ref1, _ref2;
      expectation = null;
